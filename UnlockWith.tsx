...
const UnlockWith: React.FC = () => {
  ...
  const { setWalletsInitialized, isStorageEncrypted, startAndDecrypt } =
  ...
  useEffect(() => {
    ...
    const startUnlock = async () => {
      ...
      const isBiometricUseCapableAndEnabled =
        await Biometric.isBiometricUseCapableAndEnabled();
      const biometricType = isBiometricUseCapableAndEnabled
        ? await Biometric.biometricType()
        : undefined;
      console.log(
        "check status",
        storageIsEncrypted,
    ...
    if (state.isAuthenticating) {
      ...
      switch (state.auth.type) {
        case AuthType.Biometrics:
          if (
            state.auth.detail === "TouchID" ||
            state.auth.detail === "Biometrics"
          ) {
            return (
              <TouchableOpacity
