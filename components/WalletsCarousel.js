...
export const WalletCarouselItem = ({
  item,
  _,
  onPress,
  handleLongPress,
  ...
  const { width } = useWindowDimensions();
  const itemWidth = width * 0.82 > 375 ? 400 : width * 0.95;
  const isLargeScreen =
    Platform.OS === "android"
      ? isTablet()
  ...
  return (
    ...
    >
      ...
      >
        {/* <LinearGradient shadowColor={colors.blue} colors={WalletGradient.gradientsFor(item.type)} style={iStyles.grad}> */}
        <View style={iStyles.grad}>
          ...
          <View style={iStyles.iconBox}>
            ...
            <Icon name="chevron-down" type="font-awesome-5" size={16} color='white' />
          </View>
          {item.hideBalance ? (
            <BluePrivateBalance />
          ) : (
...
const WalletsCarousel = forwardRef((props, ref) => {
  ...
  const renderItem = useCallback(
    ({ item, index }) =>
      item ? (
        <WalletCarouselItem
          isSelectedWallet={
            !horizontal && selectedWallet
              ? selectedWallet === item.getID()
              : undefined
          }
          item={item}
          index={index}
  ...
  const itemWidth = width * 0.82 > 375 ? 300 : width * 0.82;
  return horizontal ? (
    <FlatList
      ref={flatListRef}
